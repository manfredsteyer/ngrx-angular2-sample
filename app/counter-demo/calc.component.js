System.register(['@ngrx/store', './counter', 'angular2/core'], function(exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __metadata = (this && this.__metadata) || function (k, v) {
        if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
    };
    var store_1, counter_1, core_1;
    var CalcComponent;
    return {
        setters:[
            function (store_1_1) {
                store_1 = store_1_1;
            },
            function (counter_1_1) {
                counter_1 = counter_1_1;
            },
            function (core_1_1) {
                core_1 = core_1_1;
            }],
        execute: function() {
            CalcComponent = (function () {
                function CalcComponent(store) {
                    this.store = store;
                    this.counter = store.select('x');
                }
                CalcComponent.prototype.increment = function () {
                    this.store.dispatch({ type: counter_1.INCREMENT });
                };
                CalcComponent.prototype.decrement = function () {
                    this.store.dispatch({ type: counter_1.DECREMENT });
                };
                CalcComponent.prototype.reset = function () {
                    this.store.dispatch({ type: counter_1.RESET });
                };
                CalcComponent = __decorate([
                    core_1.Component({
                        selector: 'my-app',
                        template: "\n      <button (click)=\"increment()\">Increment</button>\n        <div>Current Count: {{ counter | async }}</div>\n        <button (click)=\"decrement()\">Decrement</button>\n    "
                    }), 
                    __metadata('design:paramtypes', [store_1.Store])
                ], CalcComponent);
                return CalcComponent;
            }());
            exports_1("CalcComponent", CalcComponent);
        }
    }
});
//# sourceMappingURL=calc.component.js.map